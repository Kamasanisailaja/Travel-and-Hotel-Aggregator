...
INSERT INTO Users VALUES (2, 'Sailaja', 'p@ssw0rd', 'jane@example.com', 'Jane', 'Smith', '987-654-3210')
INSERT INTO Users VALUES(3, 'Mahesh', 'securepass', 'mike@example.com', 'Mike', 'Johnson', '555-123-4567')
INSERT INTO Users VALUES(4, 'Chaturya', 'pass123', 'sara@example.com', 'Sara', 'Williams', '111-222-3333')
INSERT INTO Users VALUES(5, 'vyshnavi', 'qwerty', 'alex@example.com', 'Alex', 'Davis', '444-555-6666')
INSERT INTO Users VALUES(6, 'hari', 'abc123', 'emily@example.com', 'Emily', 'Brown', '777-888-9999')
INSERT INTO Users VALUES(7, 'pavan', 'letmein', 'chris@example.com', 'Chris', 'Taylor', '222-333-4444')
INSERT INTO Users VALUES(8, 'vijay', 'password', 'lisa@example.com', 'Lisa', 'Wilson', '999-888-7777')
INSERT INTO Users VALUES(9, 'hema', 'ilovecoding', 'ryan@example.com', 'Ryan', 'Clark', '111-999-8888')
INSERT INTO Users VALUES(10, 'poornima', 'katepass', 'kate@example.com', 'Kate', 'Robinson', '888-777-6666')
INSERT INTO Users VALUES(11, 'abhi', 'peterpass', 'peter@example.com', 'Peter', 'Adams', '444-555-6666')
INSERT INTO Users VALUES(12, 'avinash', 'oliviapass', 'olivia@example.com', 'Olivia', 'Lee', '222-333-4444')
INSERT INTO Users VALUES(13, 'abhinay', 'jacobpass', 'jacob@example.com', 'Jacob', 'Garcia', '555-666-7777')
INSERT INTO Users VALUES(14, 'abhishek', 'miapass', 'mia@example.com', 'Mia', 'Hernandez', '999-888-7777')
INSERT INTO Users VALUES(15, 'vasu', 'ethanpass', 'ethan@example.com', 'Ethan', 'Lopez', '333-444-5555')
INSERT INTO Users VALUES(16, 'pallavi', 'avapass', 'ava@example.com', 'Ava', 'Gonzalez', '777-666-5555')
INSERT INTO Users VALUES(17, 'keerthi', 'liampass', 'liam@example.com', 'Liam', 'Perez', '666-777-8888')
INSERT INTO Users VALUES(18, 'kumar', 'sophiapass', 'sophia@example.com', 'Sophia', 'Taylor', '444-555-6666')
INSERT INTO Users VALUES(19, 'venu', 'lucaspass', 'lucas@example.com', 'Lucas', 'Smith', '222-333-4444')
INSERT INTO Users VALUES(20, 'tharun', 'ameliapass', 'amelia@example.com', 'Amelia', 'Johnson', '555-666-7777')
INSERT INTO Users VALUES(21, 'delhi', 'henrypass', 'henry@example.com', 'Henry', 'Davis', '888-999-0000')
INSERT INTO Users VALUES(22, 'teja', 'gracepass', 'grace@example.com', 'Grace', 'Wilson', '111-222-3333')
INSERT INTO Users VALUES(23, 'rajitha', 'noahpass', 'noah@example.com', 'Noah', 'Thompson', '777-888-9999')
INSERT INTO Users VALUES(24, 'mamatha', 'lilypass', 'lily@example.com', 'Lily', 'Harris', '555-666-7777')
INSERT INTO Users VALUES(25, 'jhansy', 'williampass', 'william@example.com', 'William', 'Anderson', '222-333-4444');
...
Retrieve the hotels with an average rating higher than 4.*/

--sql

SELECT hotel_name, AVG(rating) AS average_rating
FROM Hotels
INNER JOIN Reviews ON Hotels.hotel_id = Reviews.hotel_id
GROUP BY hotel_name
HAVING AVG(rating) > 4;


/*15.Get the average rating for each hotel and display only
those with an average rating greater than 4.*/
...
Count the number of bookings for each user.*/
...
/*18.Calculate the average rating for each hotel.*/
...
/*19.Find the maximum price per night for each room type.*/

--SQL

SELECT room_type, MAX(price_per_night) AS max_price_per_night
FROM Rooms
GROUP BY room_type;
...
20.Retrieve the hotels in ascending order of their star rating.*/
...
/*21.Get the flights sorted by the departure date in descending order.*/

--sql

SELECT flight_id, departure_date
FROM Flights
ORDER BY departure_date DESC;

/*22.Display the rooms ordered by their price per night in descending order.*/
...
/*23.Retrieve the users in alphabetical order based on their last name.*/
...
24.Retrieve the hotels where the average rating is greater
than the average rating of all hotels.*/
...
/*25.Find the users who have made bookings for hotels located 
in a specific city.*/
...
FROM Users
WHERE user_id IN
(SELECT user_id FROM Bookings WHERE room_id IN
(SELECT room_id FROM Rooms WHERE hotel_id IN 
(SELECT hotel_id FROM Hotels WHERE city = 'specific_city')));

/*26.Get the rooms where the price per night is higher 
than the average price per night of all rooms.*/
...
/*27.Retrieve the hotels with the maximum number of bookings.*/
...
GROUP BY hotel_name
HAVING COUNT(*) = (SELECT MAX(booking_count) 
FROM (SELECT COUNT(*) AS booking_count FROM Bookings GROUP BY room_id) 
AS subquery);

/*Correlated Queries:

28.Retrieve the users who have made bookings for rooms in 
the same hotel they reviewed.*/
...
WHERE hotel_id IN (SELECT hotel_id FROM Hotels 
WHERE hotel_id IN (SELECT hotel_id FROM Rooms WHERE room_id IN
(SELECT room_id FROM Bookings WHERE user_id = Users.user_id))));

/*29.Get the rooms with a price per night higher
than the average price per night of rooms in the same hotel.*/

--sql

SELECT room_id, room_type, price_per_night
FROM Rooms r1
WHERE price_per_night > (SELECT AVG(price_per_night) FROM Rooms r2 WHERE r1.hotel_id = r2.hotel_id);

/*30.Retrieve the hotels where the number of bookings 
is greater than the average number of bookings across all hotels.*/
...
INNER JOIN Bookings ON Rooms.room_id = Bookings.room_id
GROUP BY hotel_name
HAVING COUNT(*) > (SELECT AVG(booking_count)
FROM (SELECT COUNT(*) AS booking_count FROM Bookings GROUP BY room_id)
AS subquery);
/*Joins:

31.Retrieve all bookings with user and hotel information.*/

--sql

SELECT B.booking_id, U.username, H.hotel_name
FROM Bookings B
INNER JOIN Users U ON B.user_id = U.user_id
INNER JOIN Rooms R ON B.room_id = R.room_id
INNER JOIN Hotels H ON R.hotel_id = H.hotel_id;

/*32.Get the reviews with user and hotel information.*/

--sql

SELECT R.review_id, U.username, H.hotel_name
FROM Reviews R
INNER JOIN Users U ON R.user_id = U.user_id
INNER JOIN Hotels H ON R.hotel_id = H.hotel_id;
/*33.Retrieve the bookings with user, room, and hotel information.*/

--sql

SELECT B.booking_id, U.username, R.room_type, H.hotel_name
FROM Bookings B
INNER JOIN Users U ON B.user_id = U.user_id
INNER JOIN Rooms R ON B.room_id = R.room_id
INNER JOIN Hotels H ON R.hotel_id = H.hotel_id;

/*34.Get the flights with user information for flight bookings.*/

--sql

SELECT F.flight_id, U.username
FROM Flights F
INNER JOIN Flight_Bookings FB ON F.flight_id = FB.flight_id
INNER JOIN Users U ON FB.user_id = U.user_id;

/*35.Retrieve the car rentals with user information for car rental bookings.*/

--sql

SELECT CR.rental_id, U.username
FROM Car_Rentals CR
INNER JOIN Car_Rental_Bookings CRB ON CR.rental_id = CRB.rental_id
INNER JOIN Users U ON CRB.user_id = U.user_id;
/* Correlated Nested Queries:

36.Retrieve the users who have made bookings for rooms
in hotels with a rating greater than their average rating.*/
...
(SELECT user_id FROM Bookings WHERE room_id IN
(SELECT room_id FROM Rooms WHERE hotel_id IN 
(SELECT hotel_id FROM Hotels WHERE star_rating > 
(SELECT AVG(rating) FROM Reviews WHERE user_id = Users.user_id))));

/*37.Get the rooms with a price per night higher than the average
price per night of rooms in hotels with a rating greater than their 
average rating.*/
...
(SELECT AVG(price_per_night) FROM Rooms r2 WHERE
r2.hotel_id IN (SELECT hotel_id FROM Hotels WHERE star_rating >
(SELECT AVG(rating) FROM Reviews WHERE r2.hotel_id = Reviews.hotel_id)));

/*38.Retrieve the hotels with the maximum number of bookings from users 
who have made bookings for hotels in the same city.*/
...
WHERE B.user_id IN (SELECT user_id FROM Bookings
WHERE room_id IN (SELECT room_id FROM Rooms WHERE hotel_id IN 
(SELECT hotel_id FROM Hotels WHERE city = 
(SELECT city FROM Hotels WHERE hotel_id = H.hotel_id))))
GROUP BY hotel_name
HAVING COUNT(*) = (SELECT MAX(booking_count) FROM
(SELECT COUNT(*) AS booking_count FROM Bookings GROUP BY room_id) 
AS subquery);

/*39.Retrieve the users who have made bookings for rooms 
with a maximum occupancy of 2 or less.*/

--sql

SELECT user_id, first_name, last_name
FROM Users
WHERE user_id IN (SELECT user_id FROM Bookings WHERE room_id IN
(SELECT room_id FROM Rooms WHERE max_occupancy <= 2));

/*40.Get the average rating of each hotel.*/
...
/*41.Retrieve the hotels sorted by their average rating in descending order.*/

--sql

SELECT H.hotel_id, H.hotel_name, AVG(R.rating) AS average_rating
FROM Hotels H
LEFT JOIN Reviews R ON H.hotel_id = R.hotel_id
GROUP BY H.hotel_id, H.hotel_name
ORDER BY average_rating DESC;

/*42.Get the bookings made by users whose email 
...
/*43.Retrieve the bookings made for rooms with a price per 
night higher than $200.*/

--sql

SELECT B.booking_id, R.room_type, R.price_per_night
FROM Bookings B
INNER JOIN Rooms R ON B.room_id = R.room_id
WHERE R.price_per_night > 200;

/*44.Get the number of bookings made by each user.*/

--sql

SELECT U.user_id, U.username, COUNT(*) AS booking_count
FROM Users U
INNER JOIN Bookings B ON U.user_id = B.user_id
GROUP BY U.user_id, U.username;

/*45.Retrieve the hotels with the highest star rating in each city.*/

--sql

SELECT H.city, MAX(H.star_rating) AS max_star_rating
FROM Hotels H
GROUP BY H.city;

/*46.Get the bookings made by users whose first name
...
/*47.Retrieve the bookings made for rooms with a price 
per night within the range of $100 to $200.*/
...
FROM Bookings B
INNER JOIN Rooms R ON B.room_id = R.room_id
WHERE R.price_per_night BETWEEN 100 AND 200;

/*48.Get the hotels with the highest total revenue from bookings.*/

--sql

SELECT H.hotel_id, H.hotel_name, SUM(B.total_price) AS total_revenue
FROM Hotels H
INNER JOIN Rooms R ON H.hotel_id = R.hotel_id
INNER JOIN Bookings B ON R.room_id = B.room_id
GROUP BY H.hotel_id, H.hotel_name
...
/*49.Retrieve the bookings made by users whose last name is "Smith" or "Johnson".*/
...
FROM Bookings B
INNER JOIN Users U ON B.user_id = U.user_id
WHERE U.last_name IN ('Smith', 'Johnson');

/*50.Get the average price per night of each room type.*/

SELECT room_type, AVG(price_per_night) AS average_price
FROM Rooms
GROUP BY room_type;

/*51.Retrieve the bookings made for rooms with a maximum
occupancy greater than 4.*/

--sql

SELECT B.booking_id, R.room_type, R.max_occupancy
FROM Bookings B
INNER JOIN Rooms R ON B.room_id = R.room_id
WHERE R.max_occupancy > 1;



/*52.Retrieve the bookings made by users who have also made flight bookings.*/

--sql

SELECT distinct B.booking_id, U.username
FROM Bookings B
INNER JOIN Users U ON B.user_id = U.user_id
WHERE U.user_id IN (SELECT user_id FROM Flight_Bookings);

/*53.Get the hotels that have at least one room with a price per night
lower than $100.*/
...
/*54.Retrieve the bookings made for rooms with a price per night higher 
than the average price per night of all rooms.*/
...
/*55.Get the users who have made both hotel bookings and car rental bookings.*/
...
FROM Users U
INNER JOIN Bookings B ON U.user_id = B.user_id
WHERE U.user_id IN (SELECT user_id FROM Car_Rental_Bookings);
